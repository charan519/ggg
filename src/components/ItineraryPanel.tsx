...
export function ItineraryPanel({ onClose, userLocation }: ItineraryPanelProps) {
  ...
  const generateAIItinerary = async (destination: string, days: number, preferences: string) => {
    setIsLoading(true);
    try {
      // Simulate API call with a mock response
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Format the response to include proper markdown formatting
      const mockItinerary = formatAIItinerary(destination, days, preferences);
      
      setAiItinerary(mockItinerary);
    } catch (error) {
  ...
  // Helper function to format AI itinerary with proper styling
  const formatAIItinerary = (destination: string, days: number, preferences: string) => {
    ...
    <>
      ...
      {showPlanner && (
        <ItineraryPlanner 
          ...
          onGenerateAI={generateAIItinerary}
          isLoading={isLoading}
        />
      )}
    </>
